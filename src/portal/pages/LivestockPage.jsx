...
const CATS = ["Breeders", "Weaners", "Steers", "Bulls", "Heifers", "Calves"];
const TREATMENTS = ["Vaccination", "Drenching", "Tick treatment", "Marking", "Other"];
...
export default function LivestockPage(props) {
  ...
  const paddocks = useMemo(() => {
    ...
    (properties || []).forEach((p) => (p.subLocations || []).forEach((s) => {
      ...
      if (isPaddock) list.push({ id: `${p.id}:${s.id}`, propertyId: String(p.id), subId: String(s.id), name: `${p.name} - ${s.name || "Paddock"}` });
    }));
    return list;
  }, [properties]);
  const paddockMap = useMemo(() => new Map(paddocks.map((x) => [x.id, x])), [paddocks]);
  const active = useMemo(() => (livestockRecords || []).filter((r) => !r.archived), [livestockRecords]);
  const asc = useMemo(() => [...active].sort((a, b) => String(a.date || "").localeCompare(String(b.date || ""))), [active]);

  const mobs = useMemo(() => {
    const map = new Map();
    asc.forEach((r) => {
      if (r.kind === "mob_create") map.set(String(r.mobId || r.id), { mobId: String(r.mobId || r.id), mobName: r.mobName || "Mob", breed: r.breed || "", category: r.category || "Breeders", headCount: Math.max(0, n(r.headCount)), currentPaddockId: r.currentPaddockId || "", withholdingUntil: "", weights: [] });
      const m = map.get(String(r.mobId || ""));
      if (!m) return;
      if (r.kind === "movement") { m.currentPaddockId = r.toPaddockId || m.currentPaddockId; if (n(r.headCount) > 0) m.headCount = n(r.headCount); }
      if (r.kind === "treatment" && String(r.withholdingEndDate || "") > String(m.withholdingUntil || "")) m.withholdingUntil = r.withholdingEndDate;
      if (r.kind === "sale") m.headCount = Math.max(0, m.headCount - n(r.headCount));
      if (r.kind === "purchase") m.headCount += Math.max(0, n(r.headCount));
      if (r.kind === "calving") m.headCount += Math.max(0, n(r.calvesBorn) - n(r.losses));
      if (r.kind === "weight") m.weights.push({ date: r.date, avgWeight: n(r.avgWeight), bcs: n(r.bodyConditionScore) });
    });
    return [...map.values()].sort((a, b) => String(a.mobName).localeCompare(String(b.mobName)));
  }, [asc]);
  ...
  const wt = useMemo(() => {
    const series = [...(selectedMob?.weights || [])].sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")));
    if (series.length < 2) return { series, gain: 0, daysToTarget: null };
    const a = series[0], b = series[series.length - 1], days = Math.max(1, Math.floor((new Date(b.date) - new Date(a.date)) / (1000 * 60 * 60 * 24))), gain = (n(b.avgWeight) - n(a.avgWeight)) / days, rem = n(targetWeight) - n(b.avgWeight);
    return { series, gain, daysToTarget: gain > 0 && rem > 0 ? Math.ceil(rem / gain) : null };
  }, [selectedMob, targetWeight]);

  const open = (k) => { setMode(k); setF((p) => ({ ...p, kind: k, date: todayIso() })); };
  const save = async (payload) => saveLivestockRecord({ ...payload, archived: false, createdAt: payload.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString() });
  const lockUntil = (mobId, onDate) => [...active].filter((r) => r.kind === "treatment" && String(r.mobId || "") === String(mobId)).map((r) => String(r.withholdingEndDate || "")).filter(Boolean).sort().slice(-1)[0] >= String(onDate || "") ? [...active].filter((r) => r.kind === "treatment" && String(r.mobId || "") === String(mobId)).map((r) => String(r.withholdingEndDate || "")).filter(Boolean).sort().slice(-1)[0] : "";

  const submit = async () => {
    if (!mode) return;
    if (mode === "mob_create") { await save({ ...f, kind: "mob_create", mobId: `mob-${Date.now()}`, headCount: n(f.headCount) }); setMode(""); return; }
    if (mode === "movement") {
      const mob = mobs.find((m) => String(m.mobId) === String(f.mobId)); if (!mob) return;
      const dest = paddockMap.get(f.toPaddockId || "");
      const hit = (chemicalRecords || []).find((r) => !r.archived && String(r.propertyId || "") === String(dest?.propertyId || "") && (!r.subLocationId || String(r.subLocationId || "") === String(dest?.subId || "")) && String(r.withholdingEndDate || "") >= String(f.date || ""));
      if (hit) alert(`Warning: destination withholding active until ${hit.withholdingEndDate}.`);
      await save({ ...f, kind: "movement", mobName: mob.mobName, fromPaddockId: mob.currentPaddockId || f.fromPaddockId || "", headCount: n(f.headCount) }); setMode(""); return;
    }
    ...
    if (mode === "calving") { const mob = mobs.find((m) => String(m.mobId) === String(f.mobId)); if (!mob) return; await save({ ...f, kind: "calving", mobName: mob.mobName, calvesBorn: n(f.calvesBorn), losses: n(f.losses) }); setMode(""); }
  };
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      {mode && <SectionCard title={`Add ${mode.replace("_", " ")}`}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>
        <div><label style={labelStyle}>Date</label><input type="date" style={inputStyle} value={f.date} onChange={(e) => setF((p) => ({ ...p, date: e.target.value }))} /></div>
        {(mode !== "mob_create" && mode !== "purchase") && <div><label style={labelStyle}>Mob</label><select style={inputStyle} value={f.mobId} onChange={(e) => setF((p) => ({ ...p, mobId: e.target.value }))}><option value="">Select mob</option>{mobs.map((m) => <option key={m.mobId} value={m.mobId}>{m.mobName}</option>)}</select></div>}
        {mode === "mob_create" && <><div><label style={labelStyle}>Mob name</label><input style={inputStyle} value={f.mobName} onChange={(e) => setF((p) => ({ ...p, mobName: e.target.value }))} /></div><div><label style={labelStyle}>Breed</label><input style={inputStyle} value={f.breed} onChange={(e) => setF((p) => ({ ...p, breed: e.target.value }))} /></div><div><label style={labelStyle}>Head</label><input type="number" style={inputStyle} value={f.headCount} onChange={(e) => setF((p) => ({ ...p, headCount: e.target.value }))} /></div><div><label style={labelStyle}>Category</label><select style={inputStyle} value={f.category} onChange={(e) => setF((p) => ({ ...p, category: e.target.value }))}>{CATS.map((c) => <option key={c} value={c}>{c}</option>)}</select></div><div><label style={labelStyle}>Current paddock</label><select style={inputStyle} value={f.currentPaddockId} onChange={(e) => setF((p) => ({ ...p, currentPaddockId: e.target.value }))}><option value="">Unassigned</option>{paddocks.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div></>}
        {mode === "movement" && <><div><label style={labelStyle}>To paddock</label><select style={inputStyle} value={f.toPaddockId} onChange={(e) => setF((p) => ({ ...p, toPaddockId: e.target.value }))}><option value="">Select paddock</option>{paddocks.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div><div><label style={labelStyle}>Head at move</label><input type="number" style={inputStyle} value={f.headCount} onChange={(e) => setF((p) => ({ ...p, headCount: e.target.value }))} /></div></>}
        {mode === "treatment" && <><div><label style={labelStyle}>Ear tag (optional)</label><input style={inputStyle} value={f.earTag} onChange={(e) => setF((p) => ({ ...p, earTag: e.target.value }))} /></div><div><label style={labelStyle}>Treatment type</label><select style={inputStyle} value={f.treatmentType} onChange={(e) => setF((p) => ({ ...p, treatmentType: e.target.value }))}>{TREATMENTS.map((t) => <option key={t} value={t}>{t}</option>)}</select></div><div><label style={labelStyle}>Product</label><input style={inputStyle} value={f.productUsed} onChange={(e) => setF((p) => ({ ...p, productUsed: e.target.value }))} /></div><div><label style={labelStyle}>Dose</label><input style={inputStyle} value={f.dose} onChange={(e) => setF((p) => ({ ...p, dose: e.target.value }))} /></div><div><label style={labelStyle}>Withholding days</label><input type="number" style={inputStyle} value={f.withholdingDays} onChange={(e) => setF((p) => ({ ...p, withholdingDays: e.target.value }))} /></div><div><label style={labelStyle}>Operator</label><input style={inputStyle} value={f.operator} onChange={(e) => setF((p) => ({ ...p, operator: e.target.value }))} /></div></>}
        {mode === "weight" && <><div><label style={labelStyle}>Average weight (kg)</label><input type="number" style={inputStyle} value={f.avgWeight} onChange={(e) => setF((p) => ({ ...p, avgWeight: e.target.value }))} /></div><div><label style={labelStyle}>BCS 1-5</label><input type="number" min="1" max="5" style={inputStyle} value={f.bodyConditionScore} onChange={(e) => setF((p) => ({ ...p, bodyConditionScore: e.target.value }))} /></div></>}
